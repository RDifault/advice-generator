...
export default function Home() {
  ...
  const hoverBlur = () => {
    ...
    dice.addEventListener("mouseenter", () => {
      greenBg.classList.add("blur-md");
    });
    dice.addEventListener("mouseleave", () => {
      greenBg.classList.remove("blur-md");
    });
  };
  ...
  return (
    <main className="dark-blue flex flex-wrap items-center justify-center h-screen w-screen">
      <div className="sm:w-[90vw] lg:w-[700px] h-auto grayish-blue-dark rounded-xl flex flex-wrap flex-col justify-baseline items-center">
        <p className="text-center neon-green m-10 font-bold tracking-[0.35rem] pt-4">
        ...
        <button
          ...
          className="w-[65px] h-[65px] neon-green-bg flex items-center justify-center rounded-full relative top-[35px]"
          aria-label="Fetch new advice"
        >
          <div
            className="z-1 w-[65px] h-[65px] neon-green-bg flex items-center justify-center rounded-full hover:blur-md absolute"
            id="green-bg"
          ></div>
          <Image
            onMouseEnter={hoverBlur}
